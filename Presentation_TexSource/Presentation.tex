...
\end{itemize}
\begin{equation}
\left\langle A(x) \right\rangle=\dfrac{1}{Z}\int dx\;A(x)exp\left[ -\frac{E(x)}{k_{b}T} \right]
\end{equation}

\begin{equation}
Z=\int dx_{s}exp\left[  -\frac{E(x)}{k_{b}T}  \right]
\end{equation}
...
\begin{itemize}
\item An array of atoms that can take states $\pm 1$. The energy of the system is given by $E(\textbf{x},J)= - \left[\dfrac{1}{2}\sum_{m,n}J_{mn}x_{m}x_{n}\right]$ where J is the coupling constant between two neighbour sites.
\item The probability of the system to be in the state x is given by $p(x|\beta,J)=\frac{1}{Z(\beta,J)}\exp[-\beta E(x,J)]$ (canonical ensemble) where $\beta=1/k_{b}T\quad Z(\beta,J)=\sum_{x}exp\left[-\beta E(x,J)\right]$
...
\begin{equation*}
g(m)=\left\langle \sigma_{i}\sigma_{i+m}  \right\rangle $ 
\end{equation*}
...
\item \textbf{REMARK}: T is given in Kelvin not in Celsius (therefore it can be only $\geq$ 0)
\end{itemize}
\end{frame}
...
\item \textbf{OUTLOOK}: perform different simulations (e.g. 1000) for large lattices with 0 or low temperature in order to better exploits, with a cluster analysis, the ground/trap states. However for this purpose is better to use or C++ or Fortran since they have better performances in terms of computational cost 
\end{itemize}
...
\begin{frame}{Connections with graphical models and multivariate Gaussian   \cite{murphy2012machine}}
\begin{itemize}
...
\end{itemize}
\begin{equation*}
\log p(\textbf{y})\sim -\sum_{s\sim t}y_{s}w_{st}y_{t}=(-1/2)\textbf{y}^{T}\textbf{W}\textbf{t}+\sum_{s}b_{s}y_{s}=(-1/2)\textbf{y}^{T}\textbf{W}\textbf{y}+\textbf{b}^{T}\textbf{y}
\end{equation*}
\begin{itemize}
